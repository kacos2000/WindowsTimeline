...
#Hash table with Known Folder GUIDs & GUIDs seen to represent for Mozilla Firefox
             $known = @{"308046B0AF4A39CB" = "Mozilla Firefox";
                        "E7CF176E110C211B" = "Mozilla Firefox";
...
$Output = foreach ($item in $dbresults ){$rb++
                    ...
                    [PSCustomObject]@{
                                ...
                                DeviceType = if($rin.Type -eq 15){"Laptop"}elseif($rin.Type -eq 9){"Desktop PC"}else{$rin.Type}
                                Name = $rin.Name
                                Make = $rin.Make
                                Model = $rin.Model
                                }
