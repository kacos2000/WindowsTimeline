...
SELECT -- This the ActivityOperation Table Query
	...
	case 
		when ActivityOperation.ActivityType in (11,12,15) then ActivityOperation.Payload
   ...
   substr(hex(Activity_PackageId.ActivityId), 21, 12) || '}' as 'ID', 
   	case 
		when ActivityOperation.ClipboardPayload not NULL 
		then json_extract(ActivityOperation.ClipboardPayload,'$') 
		else ''
	end as 'ClipboardPayload',
   ...
   then json_extract(Activity.Payload, '$.appDisplayName') 
    ...
    Activity.PackageIdHash as 'PackageIdHash',
		 '{' || substr(hex(Activity_PackageId.ActivityId), 1, 8) || '-' ||
				...
				substr(hex(Activity_PackageId.ActivityId), 21, 12) || '}' as 'ID',
	case 
		when Activity.ClipboardPayload not NULL 
		then json_extract(Activity.ClipboardPayload,'$') 
		else ''
	end as 'ClipboardPayload',
